"""
Modelos ORM da aplicação de screening de CVs.

Define as entidades principais: Process, Candidate, Result, e ProcessingError.
Usa SQLAlchemy 2.0 com type hints Mapped para melhor segurança de tipos.
"""
...
class Process(Base):
    ...
    status: Mapped[ProcessStatus] = mapped_column(
        Enum(ProcessStatus), nullable=False, default=ProcessStatus.CREATED, index=True
    )
...
class Candidate(Base):
    ...
    parse_status: Mapped[ParseStatus] = mapped_column(
        Enum(ParseStatus), nullable=False, default=ParseStatus.PENDING, index=True
    )
    ...
    # Relacionamentos
    process: Mapped["Process"] = relationship(back_populates="candidates")
...
class RefreshToken(Base):
    """
    Representa um refresh token armazenado no banco de dados.

    Cada refresh token é hashed antes de ser guardado (nunca plaintext).
    O utilizador pode ter múltiplos refresh tokens ativos (diferentes dispositivos).

    Quando o utilizador faz logout, o token é marcado como is_revoked=True
    e adicionado à TokenBlacklist até expirar.

    Ciclo de vida:
    1. Gerado em POST /auth/login, guardado com hash
    2. Validado em POST /auth/refresh
    3. Revogado em POST /auth/logout, adicionado a TokenBlacklist
    4. Expirado após 7 dias (limpeza manual ou automática)
    """
...
class TokenBlacklist(Base):
    """
    Blacklist de tokens revogados (access + refresh).

    Quando um utilizador faz logout, ambos tokens são hashed e adicionados aqui.
    A validação de tokens verifica primeiro se estão em blacklist (logout imediato).

    Este registo expira naturalmente após o token expirar (cleanup automático).
    Não há relação com User porque o objetivo é apenas invalidar tokens,
    independentemente de qual utilizador era.
    """
